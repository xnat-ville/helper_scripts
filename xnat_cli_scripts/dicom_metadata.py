...
def extract_metadata(args) -> None:
    ...
    with open(args.filename, 'rb') as infile:
        ...
        if 'OriginalAttributesSequence' in ds:
            ...
            for t in [0x00100010, 0x00101000, 0x00100020, 0x00101001, 0x00100030]:
                value_string = ""
                for item in original_attributes_sequence:
                    modified_attributes_sequence = item[0x0400, 0x0550]
                    item_ds = modified_attributes_sequence[0]
                    gggg = (t >> 16) & 0xffff
                    eeee = (t)       & 0xffff
                    if t in item_ds:
                        v = item_ds[gggg,eeee]
                        value_string = value_string + repr(v.value)
